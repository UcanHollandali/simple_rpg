...
const TYPE_SIDE_MISSION: String = "side_mission"
...
const BLACKSMITH_WEAPON_UPGRADE_COST: int = 8
const BLACKSMITH_ARMOR_UPGRADE_COST: int = 6
const BLACKSMITH_REPAIR_COST: int = 5
const BLACKSMITH_TARGET_PAGE_SIZE: int = 2
const REST_HEAL_AMOUNT: int = 8
const REST_HUNGER_DELTA: int = -4
...
func setup_for_type(
	...
	match support_type:
		...
		TYPE_MERCHANT:
			...
			offers = _build_merchant_offers(stage_index)
		TYPE_SIDE_MISSION:
			_setup_side_mission(persisted_node_state, map_runtime_state)
		_:
			title_text = "Support Interaction"
			summary_text = ""
			offers = []

	if support_type != TYPE_SIDE_MISSION:
...
func load_from_save_dict(save_data: Dictionary) -> void:
	support_type = String(save_data.get("support_type", ""))
...
func _setup_side_mission(persisted_node_state: Dictionary, map_runtime_state: MapRuntimeState = null) -> void:
	var loader: ContentLoader = ContentLoaderScript.new()
	mission_definition_id = String(persisted_node_state.get("mission_definition_id", SIDE_MISSION_DEFAULT_DEFINITION_ID)).strip_edges()
	...
	title_text = String(display.get("name", "Side Mission"))

	match mission_status:
		SIDE_MISSION_STATUS_OFFERED:
			var has_target_candidates: bool = true
			if map_runtime_state != null:
				has_target_candidates = not map_runtime_state.list_eligible_side_mission_target_node_ids(source_node_id).is_empty()
			summary_text = String(rules.get("briefing_text", "Take the contract and hunt the marked enemy."))
			offers = [
				{
		...
		SIDE_MISSION_STATUS_ACCEPTED:
			summary_text = String(rules.get("accepted_text", "The mark is on the map. Kill the target, then return here."))
			offers = [
				{
		...
		SIDE_MISSION_STATUS_COMPLETED:
			summary_text = String(rules.get("completed_text", "The target is down. Claim one payment item."))
			offers = _build_side_mission_reward_offers(loader, reward_offers)
		_:
			summary_text = String(rules.get("claimed_text", "The contract board hangs empty."))
			offers = [
				{
...
func _build_side_mission_reward_offers(loader: ContentLoader, authored_offers: Array[Dictionary]) -> Array[Dictionary]:
	var result: Array[Dictionary] = []
	for authored_offer in authored_offers:
		...
		if definition_id.is_empty():
			continue
		var definition_family: String = "Weapons" if inventory_family == InventoryState.INVENTORY_FAMILY_WEAPON else "Armors"
...
func _build_merchant_offers(stage_index: int = 1) -> Array[Dictionary]:
	var loader: ContentLoader = ContentLoaderScript.new()
	var result: Array[Dictionary] = []
	for stock_entry in _load_merchant_stock_entries(loader, stage_index):
...
func _load_merchant_stock_entries(loader: ContentLoader, stage_index: int = 1) -> Array[Dictionary]:
	var merchant_stock_definition: Dictionary = loader.load_definition(MERCHANT_STOCK_FAMILY, _resolve_merchant_stock_id(stage_index))
	var rules: Dictionary = merchant_stock_definition.get("rules", {})
	return _extract_offer_array(rules.get("stock", []))


func _resolve_merchant_stock_id(stage_index: int) -> String:
	var normalized_stage_index: int = max(1, stage_index)
	return String(MERCHANT_STOCK_IDS_BY_STAGE.get(normalized_stage_index, MERCHANT_STOCK_ID))
