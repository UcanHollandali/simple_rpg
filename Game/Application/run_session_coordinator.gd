...
func get_event_state() -> EventStateScript: return event_state
func get_reward_state() -> RewardState: return reward_state
func get_level_up_state() -> LevelUpState: return level_up_state
func get_support_interaction_state() -> SupportInteractionState: return support_interaction_state
func get_last_run_result() -> String: return last_run_result
...
func build_combat_setup_data() -> Dictionary:
	...
	var setup_data: Dictionary = RunSessionStateHelperScript.build_combat_setup_data(
		active_run_state,
	...
	if not bool(setup_data.get("ok", false)):
		return setup_data
	_last_combat_reward_context = setup_data.get("combat_reward_context", {})
	setup_data.erase("combat_reward_context")
	return setup_data
...
func toggle_inventory_equipment(slot_id: int, discard_slot_id: int = -1) -> Dictionary:
	var active_run_state: RunState = ensure_run_state_initialized()
	if active_run_state == null:
		return _error_result("missing_run_state", {"slot_id": slot_id})
...
func move_inventory_slot(slot_id: int, target_index: int) -> Dictionary:
	var active_run_state: RunState = ensure_run_state_initialized()
	if active_run_state == null:
		return _error_result("missing_run_state", {"slot_id": slot_id, "target_index": target_index})
...
func use_inventory_consumable(slot_id: int) -> Dictionary:
	...
	if active_run_state == null:
		return _error_result("missing_run_state", {"slot_id": slot_id})
	return RunSessionStateHelperScript.use_inventory_consumable(active_run_state, inventory_actions, slot_id)
...
func choose_level_up_option(option_id: String) -> Dictionary:
	var active_run_state: RunState = ensure_run_state_initialized()
	if active_run_state == null:
		return _error_result("missing_run_state", {"option_id": option_id})
	if level_up_state == null:
		return _error_result("missing_level_up_state", {"option_id": option_id})
...
func choose_support_action(action_id: String, discard_slot_id: int = -1) -> Dictionary:
	var active_run_state: RunState = ensure_run_state_initialized()
	if active_run_state == null:
		return _error_result("missing_run_state", {"action_id": action_id})
...
func restore_pending_states_for_snapshot(active_flow_state: int, snapshot: Dictionary) -> Dictionary:
	...
	if active_flow_state == FlowStateScript.Type.REWARD:
		...
		if typeof(reward_variant) != TYPE_DICTIONARY:
			return _error_result("missing_reward_state")
	...
	if active_flow_state == FlowStateScript.Type.LEVEL_UP:
		...
		if typeof(level_up_variant) != TYPE_DICTIONARY:
			return _error_result("missing_level_up_state")
	...
	if active_flow_state == FlowStateScript.Type.SUPPORT_INTERACTION:
		...
		if typeof(support_interaction_variant) != TYPE_DICTIONARY:
			return _error_result("missing_support_interaction_state")
...
func _open_event_state(
	...
	event_state = RunSessionStateHelperScript.build_event_state(
		run_state,
		node_id,
		source_context,
		mark_source_node_resolved
	)
	if event_state == null:
		return false
...
func _has_eligible_roadside_template(active_run_state: RunState, target_node_id: int) -> bool:
	return RunSessionStateHelperScript.has_eligible_roadside_template(active_run_state, target_node_id)

func _open_reward_state(source_context: String, mark_current_node_resolved: bool = false) -> void:
	reward_state = RunSessionStateHelperScript.build_reward_state(
		run_state,
		source_context,
		_last_combat_reward_context,
		mark_current_node_resolved
	)
	_request_transition(FlowStateScript.Type.REWARD)


func _open_level_up_state(source_context: String) -> void:
	level_up_state = RunSessionStateHelperScript.build_level_up_state(
		run_state,
		level_up_offer_window_policy,
		source_context
	)
...
func _open_support_interaction_state(support_type: String, node_id: int) -> void:
	support_interaction_state = RunSessionStateHelperScript.build_support_interaction_state(
		run_state,
		support_type,
		node_id,
		NODE_FAMILY_HAMLET,
...
func _persist_active_support_node_state() -> void:
	RunSessionStateHelperScript.persist_active_support_node_state(
		run_state,
		support_interaction_state,
		NODE_FAMILY_HAMLET
	)
...
func _complete_noncombat_side_quest_target_on_arrival(
	...
	RunSessionStateHelperScript.complete_noncombat_side_quest_target_on_arrival(
		active_run_state,
		map_runtime_state,
		target_node_id,
		target_node_type,
		DIRECT_COMBAT_NODE_FAMILIES,
		inventory_actions
	)


func _apply_side_quest_completion_hooks(active_run_state: RunState, completed_side_quest_state: Dictionary) -> void:
	RunSessionStateHelperScript.apply_side_quest_completion_hooks(
		active_run_state,
		completed_side_quest_state,
		inventory_actions
	)


func _build_pending_node_compat_mirror() -> Dictionary:
	if run_state == null or run_state.map_runtime_state == null or not run_state.map_runtime_state.has_pending_node():
		return {}
	return {
		APP_STATE_KEY_PENDING_NODE_ID: int(run_state.map_runtime_state.pending_node_id),
		APP_STATE_KEY_PENDING_NODE_TYPE: String(run_state.map_runtime_state.pending_node_type),
	}


func _restore_pending_node_compat_mirror(app_state: Dictionary) -> void:
