...
const ThemeTokensScript = preload("res://Game/UI/temp_screen_theme_tokens.gd")
const StyleBuilderScript = preload("res://Game/UI/temp_screen_theme_style_builder.gd")

# Temporary filename retained intentionally; this script is the shared UI token/style/readability facade.
const PANEL_FILL_COLOR := ThemeTokensScript.PANEL_FILL_COLOR
const PANEL_SOFT_FILL_COLOR := ThemeTokensScript.PANEL_SOFT_FILL_COLOR
const PANEL_BORDER_COLOR := ThemeTokensScript.PANEL_BORDER_COLOR
const TEAL_ACCENT_COLOR := ThemeTokensScript.TEAL_ACCENT_COLOR
const RUST_ACCENT_COLOR := ThemeTokensScript.RUST_ACCENT_COLOR
const REWARD_ACCENT_COLOR := ThemeTokensScript.REWARD_ACCENT_COLOR
const TEXT_PRIMARY_COLOR := ThemeTokensScript.TEXT_PRIMARY_COLOR
const TEXT_MUTED_COLOR := ThemeTokensScript.TEXT_MUTED_COLOR
const TEXT_SUBTLE_COLOR := ThemeTokensScript.TEXT_SUBTLE_COLOR
const DISABLED_TEXT_COLOR := ThemeTokensScript.DISABLED_TEXT_COLOR
const BACKDROP_SCRIM_COLOR := ThemeTokensScript.BACKDROP_SCRIM_COLOR
const REGULAR_STACK_SPACING_SHORT := ThemeTokensScript.REGULAR_STACK_SPACING_SHORT
const REGULAR_STACK_SPACING_TALL := ThemeTokensScript.REGULAR_STACK_SPACING_TALL
const MENU_STACK_SPACING_SHORT := ThemeTokensScript.MENU_STACK_SPACING_SHORT
const MENU_STACK_SPACING_TALL := ThemeTokensScript.MENU_STACK_SPACING_TALL
const MIN_READABLE_LABEL_FONT_SIZE := ThemeTokensScript.MIN_READABLE_LABEL_FONT_SIZE
const MIN_DENSE_LABEL_FONT_SIZE := ThemeTokensScript.MIN_DENSE_LABEL_FONT_SIZE
const MIN_VALUE_LABEL_FONT_SIZE := ThemeTokensScript.MIN_VALUE_LABEL_FONT_SIZE
const MIN_BUTTON_FONT_SIZE := ThemeTokensScript.MIN_BUTTON_FONT_SIZE
const MIN_TOUCH_TARGET_HEIGHT := ThemeTokensScript.MIN_TOUCH_TARGET_HEIGHT
const MIN_SMALL_BUTTON_WIDTH := ThemeTokensScript.MIN_SMALL_BUTTON_WIDTH
const MIN_RUNTIME_ICON_SIZE := ThemeTokensScript.MIN_RUNTIME_ICON_SIZE
const MIN_BUTTON_ICON_SIZE := ThemeTokensScript.MIN_BUTTON_ICON_SIZE
const MIN_INVENTORY_ICON_SIZE := ThemeTokensScript.MIN_INVENTORY_ICON_SIZE
const SURFACE_SCRIM_ALPHA_BY_KEY := ThemeTokensScript.SURFACE_SCRIM_ALPHA_BY_KEY
const SURFACE_TOKENS_BY_KEY := ThemeTokensScript.SURFACE_TOKENS_BY_KEY
const DEFAULT_PANEL_STYLE_TOKENS := ThemeTokensScript.DEFAULT_PANEL_STYLE_TOKENS
const COMPACT_STATUS_PANEL_TOKENS := ThemeTokensScript.COMPACT_STATUS_PANEL_TOKENS
const CHOICE_CARD_PANEL_TOKENS := ThemeTokensScript.CHOICE_CARD_PANEL_TOKENS
const INVENTORY_PANEL_TOKENS_BY_DENSITY := ThemeTokensScript.INVENTORY_PANEL_TOKENS_BY_DENSITY
const LARGE_BUTTON_STYLE_TOKENS := ThemeTokensScript.LARGE_BUTTON_STYLE_TOKENS
const SMALL_BUTTON_STYLE_TOKENS := ThemeTokensScript.SMALL_BUTTON_STYLE_TOKENS


static func resolve_surface_tokens(surface_key: String, band: String) -> Dictionary:
	return ThemeTokensScript.resolve_surface_tokens(surface_key, band)


static func resolve_surface_scrim_alpha(surface_key: String, fallback: float = 0.38) -> float:
	return ThemeTokensScript.resolve_surface_scrim_alpha(surface_key, fallback)


static func apply_portrait_safe_margins(
	margin: MarginContainer,
	max_content_width: int = 960,
	min_side_margin: int = 28,
	top_margin: int = 30,
	...
	var target_content_width: float = max(
		0.0,
		min(float(max_content_width), viewport_width - float(min_side_margin * 2))
	)
	var side_margin: int = min_side_margin
	if viewport_width > 0.0 and target_content_width > 0.0:
		side_margin = max(
			min_side_margin,
			int(floor((viewport_width - target_content_width) * 0.5))
		)
...
static func ensure_shell(
	parent: Control,
	target: Control,
	shell_name: String,
	pad_x: float = 12.0,
	pad_y: float = 12.0,
	accent: Color = PANEL_BORDER_COLOR
) -> PanelContainer:
...
static func apply_panel(
	...
	if panel == null:
		return
	StyleBuilderScript.apply_panel_style(
		panel,
		PANEL_FILL_COLOR,
		accent,
		corner_radius,
		fill_alpha,
		DEFAULT_PANEL_STYLE_TOKENS
	)


static func apply_chip(
	panel: PanelContainer,
	label: Label,
	accent: Color = TEAL_ACCENT_COLOR
) -> void:
	if panel == null or label == null:
		return
	panel.add_theme_stylebox_override(
		"panel",
		StyleBuilderScript.build_chip_style(PANEL_SOFT_FILL_COLOR, accent)
	)
...
static func apply_compact_status_area(
	panel: PanelContainer,
	accent: Color = PANEL_BORDER_COLOR
) -> void:
	if panel == null:
...
static func apply_choice_card_shell(
	panel: PanelContainer,
	accent: Color = PANEL_BORDER_COLOR
) -> void:
	if panel == null:
...
static func apply_inventory_section_panel(
	panel: PanelContainer,
	accent: Color = PANEL_BORDER_COLOR,
	...
	if panel == null:
		return
	var tokens: Dictionary = ThemeTokensScript.resolve_inventory_panel_tokens(density)
	apply_panel(
		panel,
		accent,
...
static func apply_inventory_section_text(
	title_label: Label,
	hint_label: Label,
	tone: String,
	density: String = "standard"
...
static func apply_status_chip_shell(
	panel: PanelContainer,
	accent: Color = TEAL_ACCENT_COLOR,
	...
	panel.add_theme_stylebox_override(
		"panel",
		StyleBuilderScript.build_status_chip_style(PANEL_SOFT_FILL_COLOR, accent, density)
	)


static func apply_status_progress_bar(
	bar: ProgressBar,
	accent: Color = TEAL_ACCENT_COLOR
) -> void:
	if bar == null:
	...
	bar.add_theme_stylebox_override(
		"background",
		StyleBuilderScript.build_progress_bar_background_style(PANEL_SOFT_FILL_COLOR)
	)
	bar.add_theme_stylebox_override(
		"fill",
		StyleBuilderScript.build_progress_bar_fill_style(accent)
	)


static func resolve_status_accent(
	semantic: String,
	fallback: Color = PANEL_BORDER_COLOR
) -> Color:
	match semantic:
		"health", "danger":
...
static func clamp_runtime_icon_size(
	size: float,
	minimum: float = MIN_RUNTIME_ICON_SIZE
) -> float:
	if size <= 0.0:
		return size
...
static func apply_value_label(
	...
	label.add_theme_font_size_override(
		"font_size",
		clamp_readable_value_font_size(resolved_font_size)
	)
...
static func apply_button(
	...
	StyleBuilderScript.apply_button_state_overrides(
		button,
		accent,
		normal_fill,
		hover_fill,
	...
	button.add_theme_constant_override(
		"icon_max_width",
		clamp_button_icon_size(int(tokens.get("icon_max_width", 24)))
	)
	button.add_theme_constant_override("outline_size", 0)
	apply_font_role(button, UiTypographyScript.ROLE_BUTTON)
	button.add_theme_font_size_override(
		"font_size",
		clamp_button_font_size(UiTypographyScript.DEFAULT_BUTTON_SIZE)
	)
	button.focus_mode = Control.FOCUS_ALL
	button.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND


static func apply_small_button(
	button: Button,
	accent: Color = PANEL_BORDER_COLOR,
	...
	button.add_theme_constant_override(
		"icon_max_width",
		clamp_button_icon_size(int(tokens.get("icon_max_width", 18)))
	)
	apply_font_role(button, UiTypographyScript.ROLE_BUTTON)
	button.add_theme_font_size_override(
		"font_size",
		clamp_button_font_size(UiTypographyScript.DEFAULT_SMALL_BUTTON_SIZE)
	)
	button.focus_mode = Control.FOCUS_ALL
	button.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
...
static func apply_wayfinder_backdrop(
	root: Control,
	far_alpha: float = 0.52,
	mid_alpha: float = 0.24,
	overlay_alpha: float = 0.16,
...
static func apply_modal_popup_shell(
	root: Control,
	margin: MarginContainer,
	content: Control,
	accent: Color = PANEL_BORDER_COLOR,
	shell_name: String = "ContentShell",
	margin_left: int = 36,
	margin_top: int = 104,
	margin_right: int = 36,
	margin_bottom: int = 104
) -> PanelContainer:
...
static func intensify_panel(
	panel: PanelContainer,
	accent: Color,
	border_width: int = 2,
	shadow_size: int = 20,
	fill_boost: float = 0.02,
	...
	panel.add_theme_stylebox_override(
		"panel",
		StyleBuilderScript.build_tuned_panel_style(
			existing_style,
			accent,
			border_width,
			shadow_size,
...
static func apply_font_role(
	control: Control,
	role: String = UiTypographyScript.ROLE_BODY
) -> void:
	if control == null:
		return
...
static func compute_overlay_margins(
	viewport_size: Vector2,
	max_content_width: int = 920,
	min_side_margin: int = 30
) -> Dictionary:
	...
	if viewport_size.x > 0.0:
		var target_width: float = min(
			...
			viewport_size.x - float(min_side_margin * 2)
		)
		side_margin = max(
			min_side_margin,
			int(floor((viewport_size.x - target_width) * 0.5))
		)
	return {
		"left": side_margin,
		"top": top_margin,
