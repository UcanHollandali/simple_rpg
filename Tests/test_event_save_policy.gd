...
func test_event_state_save_is_explicitly_unsupported() -> void:
	...
	assert(not bool(snapshot_result.get("ok", true)), "Expected Event flow snapshots to stay unsupported in the current save-safe baseline.")
	assert(String(snapshot_result.get("error", "")) == "unsupported_save_state", "Expected Event flow snapshot attempts to fail with unsupported_save_state.")
	...
	assert(not bool(validation_result.get("ok", true)), "Expected SaveService validation to reject Event snapshots.")
	assert(String(validation_result.get("error", "")) == "unsupported_save_state", "Expected Event snapshot validation to fail with unsupported_save_state.")
