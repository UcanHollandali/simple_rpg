...
func _init() -> void:
	...
	test_valid_transition_node_resolve_to_event()
...
func test_valid_transition_node_resolve_to_event() -> void:
	var manager = GameFlowManagerScript.new()
	...
	assert(
		...
		"Expected NODE_RESOLVE -> EVENT to be valid."
	)
	manager.free()
...
func test_save_safe_naming_split() -> void:
	...
	for state in [
		...
		FlowStateScript.Type.RUN_SETUP,
