...
func test_bleed_applies_ticks_and_expires_in_combat_state() -> void:
	...
	assert(not bool(brace_result.get("skipped", true)), "Expected brace to resolve before the first bleed attack.")

	var first_enemy_result: Dictionary = flow.process_enemy_action()
	assert(int(first_enemy_result.get("damage_applied", -1)) == 2, "Expected brace to reduce the first bleed hit from 3 to 2.")
	assert(flow.combat_state.player_hp == 58, "Expected player HP to drop from 60 to 58 after the first bleed hit.")
	...
	assert(flow.combat_state.player_hp == 57, "Expected bleed to tick for 1 damage at the first turn end.")
	assert(
	...
	assert(flow.combat_state.player_hp == 55, "Expected second bleed tick to land after the second enemy action.")
	assert(
	...
	assert(flow.combat_state.player_hp == 53, "Expected third bleed tick to apply before the status expires.")
	...
	assert(run_state.player_hp == 53, "Expected committed run HP to reflect combat-local bleed damage.")
