...
func test_poison_applies_ticks_and_expires_in_combat_state() -> void:
	...
	assert(not bool(brace_result.get("skipped", true)), "Expected brace to resolve before the first poison attack.")

	var enemy_result: Dictionary = flow.process_enemy_action()
	assert(int(enemy_result.get("damage_applied", -1)) == 2, "Expected brace to reduce the first venom hit from 4 to 2.")
	...
	assert(int(second_enemy_result.get("damage_applied", -1)) == 1, "Expected brace to reduce the second venom enemy hit from 2 to 1.")

	var second_turn_end: Dictionary = flow.process_turn_end()
	assert(flow.combat_state.player_hp == 53, "Expected second poison tick to apply before the status expires.")
	...
	assert(run_state.player_hp == 53, "Expected committed run HP to reflect combat-local poison damage.")
