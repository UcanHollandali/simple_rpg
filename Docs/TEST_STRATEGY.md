...
- macOS/Linux architecture guard validator: `python3 Tools/validate_architecture_guards.py`
  - semantic hard-fail guards cover owner/truth/coupling drift, compatibility creep, facade/public-surface widening, live contract drift, and catalog or legacy-surface regressions
  - ergonomic warnings cover hotspot file growth and active workflow-doc ballast so maintainability pressure stays visible without pretending those warnings are gameplay-rule failures
