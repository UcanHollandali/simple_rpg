...
2. Prompt `2` - display-name helper + presenter wiring
3. Prompt `3` - topology refactor
4. Prompt `4` - reconnect tuning
5. Prompt `5` - placement tuning
6. Prompt `6` - composer path-family differentiation
7. local asset production in `AI_ASSET_ROADMAP_V2.md`
8. Prompt `7` - asset hook wiring
9. Prompt `8` - variation verification + residue cleanup
10. much later, separate chat: `MAP_RUNTIME_STATE_EXTRACTION_PLAN.md`
...
## Prompt 2 - Display-Name Helper + Presenter Wiring

```text
Display-name helper and presenter wiring step for simple_rpg's map pass. Single presentation-only code step.
...
2. Wire that helper into every Game/UI presenter / shell file identified by Prompt 1 that currently shows family labels to the player.
...
1. `MAP_MASTER_PROMPTS.md` for Prompts 1 -> 6
