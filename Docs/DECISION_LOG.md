...
| `D-036` | Current combat foundation uses `Attack`, `Defend`, and `Use Item`; `Defend` creates temporary `Guard`, shields strengthen defend guard, offhand weapons act as dual-wield modifiers, broader combat-time gear management is no longer canonical, and the only live exception is the narrow turn-consuming `SwapHand` lane for eligible carried `right_hand` / `left_hand` spares. | `COMBAT_RULE_CONTRACT.md`, `SOURCE_OF_TRUTH.md`, `GDD.md` |
