...
- Do not merge multiple parts into one large patch unless the current part proves trivial and already green.
...
### Part A - `NodeResolve` Contract

Use this when you want only the first guarded slice:

```text
Apply only Prompt 02 Part A.

Scope:
- Align the `NodeResolve` contract only.
- Keep `NodeResolve` as a transition shell.
- Make the remaining live generic fallback explicit in docs/tests/guards.
- Do not remove or change live generic fallback behavior.

Stop rule:
- If behavior would change, stop and say "escalate first".

Validation:
- validate_architecture_guards
- targeted flow/NodeResolve tests
- full suite before closing the part
```

### Part B - Application Invalid-State + Pending-Node Boundary

```text
Apply only Prompt 02 Part B.

Scope:
- Standardize application invalid-state/error handling.
- Keep the pending-node boundary explicit.
- `MapRuntimeState` stays the only pending-node owner.
- `app_state.pending_node_id` / `app_state.pending_node_type` stay compatibility mirrors only.

Stop rule:
- If cleanup would change save shape, restore behavior, or owner meaning, stop and say "escalate first".

Validation:
- validate_architecture_guards
- targeted application/save/flow tests for the touched slice
- full suite before closing the part
```

### Part C - Bootstrap Surface + Scene Shell Cleanup

```text
Apply only Prompt 02 Part C.

Scope:
- Narrow bootstrap-surface drift where ownership stays unchanged.
- Prefer typed existing surfaces over string-based reflection.
- Finish remaining scene shell/theme/layout/audio cleanup through existing helpers.

Do not:
- add new `AppBootstrap` public methods
- widen `/root/AppBootstrap` lookup spread
- move gameplay/save/map truth into scenes

Validation:
- validate_architecture_guards
- targeted scene/router/UI tests
- map and combat scene isolation
- smoke if boot/router wiring changes
- full suite before closing the part
```

### Part D - Inventory Traversal + UI Constant Cleanup

```text
Apply only Prompt 02 Part D.

Scope:
- Remove shared inventory-panel traversal hotspots.
- Centralize portrait density/theme/accessibility constants under one `Game/UI/` owner.
- Keep value changes minimal unless explicitly justified.

Do not:
- widen runtime state
- move ownership
- grow hotspot files past guard caps

Validation:
- validate_architecture_guards
- targeted inventory/map/combat presenter tests
- map and combat scene isolation when applicable
- full suite before closing the part
```

### Part E - Overlay Contract Hardening + Closeout

```text
Apply only Prompt 02 Part E.

Scope:
- Harden the overlay contract.
- Keep names stable.
- Remove scene-side string choreography in favor of one typed/constant surface.
- Do optional compact accessibility/tooling polish only if the guarded work is already green.

Stop rule:
- If the change would alter owner meaning, flow behavior, or save behavior, stop and say "escalate first".

Validation:
- validate_architecture_guards
- targeted overlay/scene tests
- relevant scene isolation
- full suite before closing the part
```
